                ...
                <div class="tablepage__content">
                    ...
                    <div class="openorder-table">
                        <div class="openorder-table__header clearfix">
                            ...
                            <h2 class="col7 text-right">Unexecuted</h2>
                            <h2 class="col8 text-right">Executed Total</h2>
                        ...
                        <div class="openorder-table__body clearfix" v-for="(order, index) in orders" :key="index">
                            <div class="openorder-table__row clearfix">
                                ...
                                <p class="col6 text-right">
                                    <span class="mobile-unit">Executed({{ order.buycoin }})</span>
                                    {{ order.trade_quantity | numberFormat('0,000.00000000') }}
                                    <span class="unit">({{ order.buycoin }})</span>
                                </p>
                                <p class="col7 text-right">
                                    <span class="mobile-unit">Unexecuted({{ order.buycoin }})</span>
                                    {{ order.unexecuted | numberFormat('0,000.00000000') }}
                                    <span class="unit">({{ order.buycoin }})</span>
                                </p>
                                <p class="col8 text-right">
                                    <span class="mobile-unit">Amount({{ order.sellcoin }})</span>
                                    {{ order.exetotal | numberFormat('0,000.00000000') }}
                                    <span class="unit">({{ order.sellcoin }})</span>
                                </p>
                                <p class="col9 text-right">
                                    <a href="#//" class="textbutton disabled">Cancel</a>
                                </p>
                            </div>
                        </div>
                    </div>
